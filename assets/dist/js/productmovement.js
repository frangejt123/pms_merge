$(document).ready(function () {
	$("#li-productmovement").addClass("active");
	...
	$("div#update_sales_modal, #new_pms_modal").on("shown.bs.modal", function () {
		$("#update_pms_sales_inp, #pms_sales")
			.on("blur", function (e) {
				var value = $(this).val();

				var actualNumber = +value.replace(/,/g, "");
				var formatted = actualNumber.toLocaleString("en-US", {
	...
	$("#print_pms_btn").on("click", function () {
		...
		window.open(
			baseurl +
				"/productmovement/report_productmovement?q=" +
				selectedid.split("_")[1],
			"_blank"
		);
	});
	...
	function parseTable(data) {
		...
		$.each(data, function (ind, row) {
			...
			li +=
				...
				row["id"] +
				'">' +
				'<td class="clickable js-pmsinfo-toggle" data-toggle="collapse" id="' +
				row["product_id"] +
				'" data-target="child' +
				row["product_id"] +
				'">' +
			...
			li +=
				...
				row["pos_total"] +
				"</td>" +
				'<td class="td_beg_data">' +
				row["beginning"] +
				"</td>" +
				'<td class="td_del_data">' +
				row["delivery"] +
				"</td>" +
				'<td class="td_transin_data">' +
				row["trans_in"] +
				"</td>" +
				'<td class="td_end_data">' +
				row["ending"] +
				"</td>" +
				'<td class="td_ret_data">' +
				row["return_stock"] +
				"</td>" +
				'<td class="td_transout_data">' +
				row["trans_out"] +
				"</td>" +
				'<td class="td_actual_data">' +
				row["actual"] +
				"</td>" +
				'<td class="td_discrepancy_data"><span class="label ' +
				label_cls +
				'">' +
				row["discrepancy"] +
				"</span></td>" +
			...
			if (row["child"].length > 0) {
				...
				li += '<th style="width: 100px"></th>' + "</thead>" + "<tbody>";
				...
				$.each(row["child"], function (cind, crow) {
					...
					for (var i = 1; i <= pos_count; i++) {
						li +=
							'<td style="padding-left: 15px;">' + crow["pos" + i] + "</td>";
					}
	...
	$("table#productmovementtable").on(
		"click",
		"td.js-pmsinfo-toggle",
		function () {
			var tgt = $(this).attr("data-target");
			var id = $(this).attr("id");
			$("table#productmovementtable")
				.find("tr#" + id)
				.css({ "font-weight": "normal", color: "#333" });
			if ($("table#productmovementtable").find("tr.collapse." + tgt).length > 0)
				if (
					$("table#productmovementtable")
						.find("tr.collapse." + tgt)
						.hasClass("uncollapse")
				) {
					$("table#productmovementtable")
						.find("tr.collapse." + tgt)
						.hide()
						.removeClass("uncollapse");
				} else {
					$("table#productmovementtable")
						.find("tr#" + id)
						.css({ "font-weight": "bold", color: "#0b8cc1" });
					$("table#productmovementtable")
						.find("tr.collapse." + tgt)
						.show()
						.addClass("uncollapse");
	...
	function changeTDtoInput(tdobj, classname) {
		...
		if (!$(that).attr("focused")) {
			$(that).html(
				"<input type='text' class='tableinput' value='" +
					value +
					"' style='border:0px; border-bottom: 1px solid #999;background: transparent;width: 40px;line-height: 15px;'>"
			);
		...
		$.ajax({
			...
			success: function (res) {
				...
				if (res["success"]) {
					...
					var spandata =
						'<span class="label ' + cls + '">' + res["discrepancy"] + "</span>";
					$("table#productmovementtable")
						.find("tr[data-period='" + id + "'] td.td_discrepancy_data")
						.html(spandata);
					$("table#productmovementtable")
						.find("tr[data-period='" + id + "'] td.td_actual_data")
						.html(res["actual"]);
				}
			},
		});
			...
			$.ajax({
				...
				success: function (data) {
					if (data == "nodata") {
						...
						// else
						$("#complete_pms_btn, #update_pms_btn, #delete_pms_btn").addClass(
							"disabled"
						);
						...
						$("#sales_div")
							.find("h3")
							.html("Sales: " + data["sales"]);
						$("#sales_div").show();

						//if(!$(this).hasClass("complete_pms"))
						$(
							"#update_sales_btn, #update_pms_btn, #delete_pms_btn"
		...
		$.ajax({
			...
			success: function (res) {
				...
				if (res["success"]) {
					...
					updatecsvfiles = [];
					$.bootstrapGrowl(
						"&nbsp; &nbsp; <span class='fa fa-exclamation-check' style='font-size: 20px'></span> &nbsp; Changes successfully save.",
						{
							type: "success",
							allow_dismiss: false,
							width: 300,
	...
	function uploadFiles(event) {
		...
		if (pms_date == "") {
			$.bootstrapGrowl(
				"&nbsp; &nbsp; <span class='fa fa-exclamation-circle' style='font-size: 20px'></span> &nbsp; Please select Period Date.",
				{
					type: "danger",
					allow_dismiss: false,
					width: 300,
		...
		if (doexists) {
			$.bootstrapGrowl(
				"&nbsp; &nbsp; <span class='fa fa-exclamation-circle' style='font-size: 20px'></span> &nbsp; Period Date already exist.",
				{
					type: "danger",
					allow_dismiss: false,
					width: 300,
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data["success"]) {
					...
					var tr =
						'<tr id="row1"><td colspan="7" align="center"><font style="color: #f30"><b>No data to display.</b></font></td></tr>';
					$("table#productmovementtable tbody").html(tr);

					$.bootstrapGrowl(
						"&nbsp; &nbsp; <span class='fa fa-check-circle' style='font-size: 20px'></span> &nbsp;Period successfully deleted.",
						{
							type: "success",
							allow_dismiss: false,
							width: 300,
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data["success"]) {
					...
					$.bootstrapGrowl(
						"&nbsp; &nbsp; <span class='fa fa-check-circle' style='font-size: 20px'></span> &nbsp;Period successfully completed.",
						{
							type: "success",
							allow_dismiss: false,
							width: 300,
	...
	$("#save_pms_sales").on("click", function () {
		...
		$.ajax({
			...
			success: function (data) {
				...
				if (data["success"]) {
					$.bootstrapGrowl(
						"&nbsp; &nbsp; <span class='fa fa-check' style='font-size: 20px'></span> &nbsp; Changes successfully saved.",
						{
							type: "success",
							allow_dismiss: false,
							width: 300,
