$(document).ready(function () {
	...
	$("button#changepassword_submitbtn").on("click", function () {
		...
		if (newpassword != confirmpwd) {
			$.bootstrapGrowl(
				"&nbsp; &nbsp; <span class='fa fa-exclamation-circle' style='font-size: 20px'></span> &nbsp; Password does not match.",
				{
					type: "danger",
					width: 300,
				}
	...
	function changepassword(data) {
		$.ajax({
			...
			success: function (res) {
				...
				if (res["success"]) {
					$.bootstrapGrowl(
						"&nbsp; &nbsp; <span class='fa ffa-check-circle' style='font-size: 20px'></span> &nbsp; Password successfully updated!. Please login again.",
						{
							type: "success",
							width: 300,
						}
